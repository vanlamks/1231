...
export default function KhachHangDashboard() {
  ...
  // ❌ Huỷ đơn bán phế liệu
  const handleHuyBan = async (id) => {
    if (!window.confirm("Bạn chắc chắn muốn huỷ đơn bán này?")) return;
    try {
      await axios.delete(`${API}/DonBanPheLieu/${id}`);
      alert("🗑️ Huỷ đơn bán thành công!");
      ...
      console.error(err);
      alert("❌ Không thể huỷ đơn bán!");
    }
  };

  // ❌ Huỷ đơn mua phế liệu
  const handleHuyMua = async (id) => {
    if (!window.confirm("Bạn chắc chắn muốn huỷ đơn mua này?")) return;
    try {
      await axios.delete(`${API}/DonMuaPheLieu/${id}`);
      alert("🗑️ Huỷ đơn mua thành công!");
                ...
                <tr key={i}>
                  ...
                  <td>
                    <button
                      ...
                      onClick={() => handleHuyBan(b.id || b.Id)}
                    >
                      ❌ Huỷ
                    </button>
                  </td>
              ...
              {lichSuMua.map((m, i) => (
                <tr key={i}>
                  ...
                  <td>
                    <button
                      className="cancel-btn"
                      onClick={() => handleHuyMua(m.id || m.Id)}
                    >
                      ❌ Huỷ
                    </button>
                  </td>
